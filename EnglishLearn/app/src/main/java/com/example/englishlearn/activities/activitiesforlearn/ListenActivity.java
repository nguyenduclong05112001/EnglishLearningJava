...
public class ListenActivity extends AppCompatActivity implements TextToSpeech.OnInitListener{
            ...
            public void onResponse(Call<List<Question>> call, Response<List<Question>> response) {
                ...
                APIServer.api.getListAnswers(part.getId()).enqueue(new Callback<List<Answer>>() {
                    ...
                    public void onFailure(Call<List<Answer>> call, Throwable t) {
                        mDialog.dismiss();
                        Toast.makeText(ListenActivity.this, t.toString(), Toast.LENGTH_LONG).show();
                    }
                });
            }
    ...
    private void mappingelements() {
        speakAgain = findViewById(R.id.al_peaker);
        lbQuestion = findViewById(R.id.al_textquestion);
        contentAnswerSeleted = findViewById(R.id.al_containAnswer);
        containChoice = findViewById(R.id.al_containChoise);
        btnComplete = findViewById(R.id.al_confirmAnswer);
        wrong = new ArrayList<>();
    ...
    private void setAnswerofQuestion(int idQuestionSelected) {
        ...
        for (Answer answer : answerList) {
            if (answer.getIdquestion() == idQuestionSelected) {
                stranswer.append(answer.getContent().trim() + " ");
            }
        }
        if (stranswer.length() < 50) {
            List<Answer> listswp = answerList;
    ...
    private void isAnswerTrue() {
        ...
        for (Answer answer : answerList) {
            if (answer.getIdquestion() == ID_QUESTION_CURRENT) {
    ...
    public void onBackPressed() {
        Dialog dialogconfirmexit = new Dialog(ListenActivity.this);
